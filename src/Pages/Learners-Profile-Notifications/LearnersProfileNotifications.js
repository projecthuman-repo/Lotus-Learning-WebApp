...
function gatherNotifications(){
    ...
    let notificationsDates=[["Today","notification1","notification2","notification3"],
          ...
          <>
            {<h2 className="notifcationDates">{Dates[0]}</h2>}
              {Dates.slice(1).map(noti => (
                <div className="noti"><NotificationBlock courseDescription={noti}/></div>
              ))}
          </>
        );
      });
...
export default function LearnersProfileNotifications(){
    ...
    return (
        <LearnersProfileTemplate childComponent={Notifications}/>
    );
