...
const DisplayAudio = ({ audio }) => {
  ...
  return (
    <div className="w-full relative border-b">
      ...
      <div className=" bg-stone-800 h-[250px] flex items-center justify-center">
        <audio
        ...
        <div style={{ userSelect: "none" }} className="w-[90%]  py-4 mx-auto">
          {/* Playback controls */}
          <div className="py-2 flex justify-center items-center text-white">
            <div className="flex items-end justify-center space-x-9 text-2xl">
              ...
              <div className="text-xl relative hoverParent-small cursor-pointer">
                ...
                <div className="hoverElement-small absolute  speed-container  left-1/2 transform -translate-x-1/2 top-[-130%] w-[200px] h-[30px] bg-stone-800 z-[40] text-xs  items-center justify-between px-2 rounded-sm">
                  <p
                    ...
                    className={`${audioSpeed === 0.25 ? 'text-blue-400 font-semibold' : ''}`}
                  >
                    0.25
                  </p>
                  <p
                    onClick={() => changePlaybackSpeed(0.5)}
                    className={`${audioSpeed === 0.5 ? 'text-blue-400 font-semibold' : ''}`}
                  >
                    0.5
                  </p>
                  <p
                    onClick={() => changePlaybackSpeed(1)}
                    className={`${audioSpeed === 1 ? 'text-blue-400 font-semibold' : ''}`}
                  >
                    1
                  </p>
                  <p
                    onClick={() => changePlaybackSpeed(1.5)}
                    className={`${audioSpeed === 1.5 ? 'text-blue-400 font-semibold' : ''}`}
                  >
                    1.5
                  </p>
                  <p
                    onClick={() => changePlaybackSpeed(1.75)}
                    className={`${audioSpeed === 1.75 ? 'text-blue-400 font-semibold' : ''}`}
                  >
                    1.75
                  </p>
                </div>
              ...
              <div
                onClick={() => playAudio()}
                className=" text-lg text-stone-800 bg-stone-50 w-10 h-10  hover:h-11 hover:w-11 rounded-full cursor-pointer flex items-center justify-center transition-all"
              >
