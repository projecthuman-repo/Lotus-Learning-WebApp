...
const ProfileDropDown = () => {
  ...
  const logout = async () => {
    ...
    const deleteFromCookies = await axios.post('http://localhost:5000/cookies/delete-user-cookie', authUser , {
      ...
      headers: {
		...
		if (deleteFromCookies.status === 200) {
		  const something = await dispatch(setUser(null));
