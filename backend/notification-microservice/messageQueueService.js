...
//This command installs the amqplib package, a library that allows Node.js to interact with RabbitMQ.
...
async function connectToQueue() {
  try {
    ...
    await channel.assertQueue(queueName, {  //Ensures that the queue exists. If it doesn't, RabbitMQ will create it
      durable: false
    });
...
// FFunction to subscribe to the queue
async function subscribeToQueue(callback) {   //This function subscribes to the queue to listen for new messages.
  const channel = await connectToQueue();

  channel.consume(queueName, (message) => {    //consume method is used to read messages from the queue. When a message is received, the callback function is triggered.
    if (message !== null) {
      console.log("Received a message from the queue:", message.content.toString());
